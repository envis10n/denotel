...
export class TelnetParser {
    ...
    public receive<T extends Iterable<number>>(bytes_in: T): TelnetEvent[] {
        ...
        for (
            const ev of result.filter((e) => e.type == EventType.Negotiation)
        ) {
            if (ev.type == EventType.Negotiation) {
                ...
                if (ev.command == telnet.Negotiation.WILL) {
                    if (opt.remote && !opt.remote_state) {
                        ...
                        result.push(
                            buildSend(
                                buildNeg(telnet.Negotiation.DO, ev.option),
                            ),
                        );
                    } else if (!opt.remote) {
                        result.push(
                            buildSend(
                                buildNeg(telnet.Negotiation.DONT, ev.option),
                            ),
                        );
                    }
                    ...
                    if (opt.local && !opt.local_state) {
                        ...
                        result.push(
                            buildSend(
                                buildNeg(telnet.Negotiation.WILL, ev.option),
                            ),
                        );
                    } else if (!opt.local) {
                        result.push(
                            buildSend(
                                buildNeg(telnet.Negotiation.WONT, ev.option),
                            ),
                        );
                    }
